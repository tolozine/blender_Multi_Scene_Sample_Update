...
class RENDER_OT_BatchUpdateSamples(bpy.types.Operator):
    ...
    bl_description = "Apply max samples and noise threshold to all scenes"
    ...
    def execute(self, context):
        ...
        for scene in bpy.data.scenes:
            if scene.render.engine == 'CYCLES':
                ...
                scene.cycles.adaptive_min_samples = max(1, max_samples // 4)  # 确保最小采样数同步调整

        self.report({'INFO'}, "采样值和阈值已更新")
        return {'FINISHED'}
